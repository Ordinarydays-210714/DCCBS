...
enum cluster_heuristics_type { N, BP, CH, CHBPNS, CHBPNM,CHBPNRM, CHBP, DCCBS};
...
public:
    ...
    // variables for DCCBS
	int search_range = 1;
	int search_decaying_rate = 0 ;
	int search_decaying_period = 10;
    int min_search_range = 1;
	...
	void init()
	{
        ...
        if(ch_type != cluster_heuristics_type::N){
            ...
            // for DCCBS
            mdd_soft_copies.resize(search_range+100);
            for (int i = 0; i < search_range+1; i++)
            {
                mdd_soft_copies[i].init(10000);
            }
...
private:
    ...
    // variables for DCCBS
    vector<SoftMDD> mdd_soft_copies;
    ...
    //Functions used in DCCBS
    int computeExpandedClusterHeuristicAndBypass(CBSNode& curr, vector<bool>& excluded_agents, double time_limit);
    result_type findExpandedClusterOrBypass(CBSNode& curr, int a_m, Path& bypass_path, vector<bool>& excluded_agents,vector<bool>& SG, double time_limit);
    bool mutexExpandCheckingWithCache(CBSNode& node, int main_agent, int check_agent, Path& main_path,bool& found_incompatible_nodes, bool& path_not_existed, int current_depth, 
                                        unordered_set<int>& cluster_set, vector<bool>& excluded_agents, vector<bool>& processed_agents, vector<bool>& SG, double time_limit);
    bool findExpandedMutexBetweenMDDs(MDD *main_mdd, MDD *secondary_mdd, vector<tuple<int, int, int>> &main_mutex, vector<tuple<int, int, int>> &secondary_mutex, Path* main_path, Path* second_path);
    bool findDeepClusterOrBypass(CBSNode& curr, int a_m, Path& bypass_path, int current_depth, unordered_set<int> CA, vector<bool>& SG, unordered_set<int>& cluster_set_from_upper_depth,
                                    vector<bool>& excluded_agents, vector<bool>& processed_agents, double time_limit);
};