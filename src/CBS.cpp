...
bool CBS::solve(double _time_limit, int _cost_lowerbound, int _cost_upperbound)
{
	...
	while (!open_list.empty() && !solution_found)
	{
		...
		//Expand the node
		num_HL_expanded++;
		if (search_decaying_period > 0)
		{
			if (search_range > 1 && num_HL_expanded % search_decaying_period == 0)
			{
				if (search_range - search_decaying_rate > min_search_range)
				{
					if (search_range - search_decaying_rate > num_of_agents)
					{
						search_range = num_of_agents;
						heuristic_helper.search_range = search_range;
					}
					else
					{
						search_range -= search_decaying_rate;
						heuristic_helper.search_range = search_range;
					}
				}
				else
				{
					search_range = min_search_range;
					heuristic_helper.search_range = search_range;
				}
			}
		}		
		...
		if (search_range == 0)
		{
			cluster_heuristics_type cht = cluster_heuristics_type::N;
			setClusterHeuristicType(cht);
		}
