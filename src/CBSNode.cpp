...
std::ostream& operator<<(std::ostream& os, const CBSNode& node)
{
	os << "Node " << node.time_generated << " (" << node.g_val + node.h_val << " = " << node.g_val << " + " <<
	   node.h_val << ", " << "cluster heuristic : " << node.cluster_heuristic << ") with " << node.conflicts.size() + node.unknownConf.size() << " conflicts and " <<
	   node.paths.size() << " new paths ";
	   os << "Cluter agents : ";
	for (auto c : node.cluster_found){
		os << "(";
		for (auto agent : c->agent_id){
			os << agent << ", ";
		}
		os << "), ";
