...
void ConstraintPropagation::get_expand_mutex_node(vector<tuple<int, int, int>> &main_mutex,
                                           vector<tuple<int, int, int>> &secondary_mutex, const Path* main_path, const Path* second_path){
  int mdd_size = min(mdd1->levels.size(),mdd0->levels.size());
  for (int i = 1; i < mdd_size; i++) {
      for (auto &curr_ptr_i: mdd0->levels[i]) {
          bool is_mutex_node = true;
          if (curr_ptr_i->location == (*main_path)[i].location && curr_ptr_i->location == (*second_path)[i].location){
            for (auto &it: mdd1->levels[i])
            {
              if (has_fwd_mutex(curr_ptr_i, it)) {
                secondary_mutex.emplace_back(make_tuple(i, it->location, -2));
                secondary_mutex.emplace_back(make_tuple(i, curr_ptr_i->location, -3));
              }
              else {
                is_mutex_node = false;
                // break;
      ...
      for (auto &curr_ptr_i:  mdd1->levels[i]) {
          bool is_mutex_node = true;
          if (curr_ptr_i->location == (*second_path)[i].location && curr_ptr_i->location == (*main_path)[i].location){
            for (auto &it: mdd0->levels[i])
            {
              if (has_fwd_mutex(curr_ptr_i, it)) {
                main_mutex.emplace_back(make_tuple(i, it->location, -2));
                main_mutex.emplace_back(make_tuple(i, curr_ptr_i->location, -3));
              }
              else {
                is_mutex_node = false;
                // break;
