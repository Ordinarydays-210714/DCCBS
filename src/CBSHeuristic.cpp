...
bool CBSHeuristic::computeInformedHeuristics(CBSNode& curr, double _time_limit)
{
    ...
    // if(search_range == 0){
    //     ch_type = cluster_heuristics_type::N;
    // }
    // else if(search_range == 1){
    //     ch_type = cluster_heuristics_type::CHBP;
    // }
    ...
    switch (ch_type) {
        ...
        case cluster_heuristics_type::DCCBS:
        {
            num_second_heuristic_computed ++;
            ch = computeExpandedClusterHeuristicAndBypass(curr,excluded_agents,time_limit);
            break;
        }
    }
...
// Expanded Cluster Heuristic
bool CBSHeuristic::findDeepClusterOrBypass(CBSNode& curr, int a_m, Path& bypass_path, int current_depth, unordered_set<int> CA, vector<bool>& SG,
                                            unordered_set<int>& cluster_set_from_upper_depth, vector<bool>& excluded_agents, 
                                            vector<bool>& processed_agents, double time_limit){
    ...
    for(;;){
        ...
        for(int conflict_agent : CA){
            ...
            if(applyMutexCache){
                // perform mutex propagation with memorization technique: a_m -> conflict_agent.
                mdd_empty = mutexExpandCheckingWithCache(curr,a_m,conflict_agent,bypass_path,found_incompatible_nodes,path_not_existed,current_depth,
                                                            cluster_set,excluded_agents,processed_agents,SG,time_limit);
            }else{
                // perform incremental mutex propagation:  a_m -> conflict_agent.
                mdd_empty = mutexChecking(curr,conflict_agent,bypass_path,found_incompatible_nodes,path_not_existed);
            }
            ...
            if(mdd_empty) {
                for(int cluster_agent : cluster_set){
                    cluster_set_from_upper_depth.insert(cluster_agent);
                }
                return true;
            }
            // break the current loop if the current path is not existed in the modified MDD.
        ...
        if(path_not_existed){
            // change the current path to an alternative path.
            path_modified = true;
            if(!mdd_soft_copies[current_depth - 1].is_labeled){
                // label all conflicts on the MDD.
                labelConflict2MDD(mdd_soft_copies[current_depth - 1], a_m);
            }

            CA.clear();
            // get an alternative path from MDD which minimize the number of conflicts, and its conflicting agents: CA.
            getPathAndConflictAgentFromMDD(mdd_soft_copies[current_depth - 1],bypass_path,CA);
            // remove the processed agent and excluded agent in CA.
            for(auto it = begin(CA); it != end(CA);)
            {
                if (processed_agents[*it] || excluded_agents[*it] || cluster_set.find(*it) != cluster_set.end())
                {
                    it = CA.erase(it);
                }
                else
...
bool CBSHeuristic::findExpandedMutexBetweenMDDs(MDD *main_mdd, MDD *secondary_mdd, vector<tuple<int, int, int>> &main_mutex,
                                        vector<tuple<int, int, int>> &secondary_mutex, Path* main_path, Path* check_path) {
    num_mutex_checking++;
    assert(main_mutex.empty() && secondary_mutex.empty());
    ...
    int mdd_size = secondary_mdd->levels.size();
    if(!cp_helper.fwd_mutexes.empty() ) {
        cp_helper.get_expand_mutex_node(main_mutex,secondary_mutex, main_path, check_path);
    }
...
bool CBSHeuristic::mutexExpandCheckingWithCache(CBSNode& node, int main_agent, int check_agent, Path& main_path,bool& found_incompatible_nodes, bool& path_not_existed, int current_depth, 
                                                unordered_set<int>& cluster_set, vector<bool>& excluded_agents, vector<bool>& processed_agents, vector<bool>& SG, double time_limit) {

    shared_ptr<mutex_nodes> main_mdd_mutex, check_mdd_mutex;

    int idx1 = main_agent * num_of_agents + check_agent;
    int idx2 = check_agent * num_of_agents + main_agent;

    // find incompatible MDD nodes from mutex graph.
    auto got_mutex1 = node.mutexGraph.find(idx1);
    auto got_mutex2 = node.mutexGraph.find(idx2);

    // copy check path
    Path check_path = *paths[check_agent];
    if (got_mutex1 != node.mutexGraph.end()){
        // get incompatible MDD nodes if exists.
        main_mdd_mutex = got_mutex1->second;
        check_mdd_mutex = got_mutex2->second;
    }
    else {
        // otherwise, check the centralized database.
        int a1, a2;
        Path* a1_path, *a2_path;
        if (main_agent < check_agent){
            a1 = main_agent;
            a2 = check_agent;
            a1_path = &main_path;
            a2_path = &check_path;
        ...
        if (got != mutexLookupTable[a1][a2].end()) // check the lookup table first
        {
            ...
            node.mutexGraph[idx3] = got->second.first;
            node.mutexGraph[idx4] = got->second.second;
            main_mdd_mutex = main_agent < check_agent ? got->second.first : got->second.second;
            check_mdd_mutex = main_agent < check_agent ? got->second.second : got->second.first;
        } else {
            // otherwise, perform mutex propagation
            auto mdd1 = mdd_helper.getMDD(node, a1, paths[a1]->size());
            auto mdd2 = mdd_helper.getMDD(node, a2, paths[a2]->size());
            ...
            //perform mutex propagation.
            if (mdd1->levels.size() < mdd2->levels.size()) {
                findExpandedMutexBetweenMDDs(mdd2, mdd1, *mdd2_mutex, *mdd1_mutex, a2_path, a1_path);
            } else {
                findExpandedMutexBetweenMDDs(mdd1, mdd2, *mdd1_mutex, *mdd2_mutex, a1_path, a2_path);
            }
            ...
            main_mdd_mutex = main_agent < check_agent ? mdd1_mutex : mdd2_mutex;
            check_mdd_mutex = main_agent < check_agent ? mdd2_mutex : mdd1_mutex;
        }
    }

    // deleting incompatible node and edges
    for( const auto& mutex : *main_mdd_mutex){
        if( get<2>(mutex) == -3){
            continue;
        }
        else if( get<2>(mutex) == -1 ) {
            // delete incompatible MDD nodes.
            for (auto &main_node_ptr: mdd_soft_copies[current_depth - 1].levels[get<0>(mutex)]) {
                if (main_node_ptr->location == get<1>(mutex)) {
                    found_incompatible_nodes = true;
                    mdd_soft_copies[current_depth - 1].deleteNode(main_node_ptr,main_path,path_not_existed);
                    break;
                }
            }
        }
        else if( get<2>(mutex) != -2 ){
            // delete incompatible MDD edges, these edges can only be singleton edges.
            for (auto &main_node_ptr: mdd_soft_copies[current_depth - 1].levels[get<0>(mutex)]) {
                if (main_node_ptr->location == get<1>(mutex)) {
                    for(auto & child_ptr : main_node_ptr->children){
                        if(child_ptr->location == get<2>(mutex)){
                            mdd_soft_copies[current_depth - 1].deleteEdge(main_node_ptr, child_ptr, main_path, path_not_existed);
                            found_incompatible_nodes = true;
                            break;
                        }
                    }
        ...
        if(mdd_soft_copies[current_depth - 1].levels[get<0>(mutex)].empty()) {
            if (found_incompatible_nodes){
                cluster_set.insert(check_agent);
            }
            //return true if MDD becomes empty
            return true;
        }
    ...
    if(current_depth < search_range && !path_not_existed){
        // copy MDD of check agent
        mdd_soft_copies[current_depth].is_labeled = false;
        mdd_soft_copies[current_depth].levels.clear();
        mdd_soft_copies[current_depth].fastCopyMDD(*mdd_helper.getMDD(node, check_agent, paths[check_agent]->size()));
        
        bool check_path_not_existed = false;
        bool check_mdd_empty = false;

        // delete incompatible MDD nodes and edges of check agent.
        for(const auto& mutex : *check_mdd_mutex){
            if(get<2>(mutex) == -2){
                for (auto &check_node_ptr: mdd_soft_copies[current_depth].levels[get<0>(mutex)]) {
                    if (check_node_ptr->location == get<1>(mutex)) {
                        mdd_soft_copies[current_depth].deleteNodeWithCheckMDD(check_node_ptr,check_path,check_path_not_existed,check_mdd_empty);
                    }
                }
            }
        }
        ...
        if(check_path_not_existed && !check_mdd_empty){
            if(!mdd_soft_copies[current_depth].is_labeled){
                labelConflict2MDD(mdd_soft_copies[current_depth], check_agent);
            }

            getPathAndConflictAgentFromMDD(mdd_soft_copies[current_depth], check_path, CA);
        ...
        else if(!check_path_not_existed){
            for(int i = check_agent*num_of_agents; i < (check_agent+1)*num_of_agents; i++){
                if(SG[i]){
                    if(!excluded_agents[i%num_of_agents]) CA.insert(i%num_of_agents);
                }
            }
        ...
        if(!check_mdd_empty){
            auto processed_agents_until_now = processed_agents;
            check_mdd_empty = findDeepClusterOrBypass(node, check_agent, check_path, current_depth + 1, CA, SG, cluster_set, excluded_agents, processed_agents_until_now, time_limit);
        }
...
result_type CBSHeuristic::findExpandedClusterOrBypass(CBSNode& curr, int a_m, Path& bypass_path, vector<bool>& excluded_agents,vector<bool>& SG, double time_limit){
    ...
    // // copy the MDD of a_m
    mdd_soft_copies[current_depth - 1].is_labeled = false;
    mdd_soft_copies[current_depth - 1].levels.clear();
    mdd_soft_copies[current_depth - 1].fastCopyMDD(*mdd_helper.getMDD(curr, a_m, paths[a_m]->size()));
...
int CBSHeuristic::computeExpandedClusterHeuristicAndBypass(CBSNode& curr, vector<bool>& excluded_agents, double time_limit){
    int cluster_heuristic = 0;
    ...
    for(;;){
        ...
        // detecting cluster and bypass of a_m, this function refers to Algorithm 2 of our paper.
        auto result = findExpandedClusterOrBypass(curr,a_m,bypass_path,excluded_agents,SG, time_limit);
        ...
        if(result == result_type::FC){
            ...
            // update the new conflicts and insert new edges to the conflict state graph SG.
            // these new conflicts are detected by our labelling method. i.e., we directly get these conflicts from MDD.
            for(auto conflict_agent : mdd_soft_copies[0].levels[mdd_soft_copies[0].levels.size()-1].front()->node_conflict_agent){
                // insert new target conflicts.
                shared_ptr<Conflict> conflict(new Conflict());
                for(int i = 0; i < paths[conflict_agent]->size(); i++){
                    if(paths[conflict_agent]->at(i).location ==mdd_soft_copies[0].levels[mdd_soft_copies[0].levels.size()-1].front()->location ){
                        if (target_reasoning)
                        {
                            conflict->targetConflict(a_m, conflict_agent,  paths[conflict_agent]->at(i).location, i);
                        }else {
            ...
            int timestep = mdd_soft_copies[0].levels.size()-2;
            SoftMDDNode* pre_node_ptr = mdd_soft_copies[0].levels[mdd_soft_copies[0].levels.size()-1].front();
            SoftMDDNode* cur_node_ptr = pre_node_ptr->pre_node;
            while(timestep >= 0 ){
                for(auto conflict_agent : cur_node_ptr->node_conflict_agent){
                    // insert new vertex conflicts
                ...
                if(cur_node_ptr != pre_node_ptr){
                    // insert new edge conflicts.
                    size_t idx = (1 + cur_node_ptr->location ) * map_size + pre_node_ptr->location;
                    if(mdd_soft_copies[0].edge_conflict_agent[timestep+1].find(idx) != mdd_soft_copies[0].edge_conflict_agent[timestep+1].end()){
                        for(auto conflict_agent : mdd_soft_copies[0].edge_conflict_agent[timestep+1][idx]){
                            shared_ptr<Conflict> conflict(new Conflict());
                            conflict->edgeConflict(a_m, conflict_agent, bypass_path[timestep].location, bypass_path[timestep+1].location, (int)timestep+1);
                            curr.unknownConf.push_back(conflict);
                            // update the conflict state graph SG
