...
int main(int argc, char** argv)
{
	...
	desc.add_options()
        ...
        ("cluster_heuristics", po::value<string>()->default_value("N"), "cluster_heuristics heuristics for the high-level search (N, BP, CH, CHBPNS, CHBPNM,CHBPNRM, CHBP, DCCBS)")
		...
		// params for DCCBS
		("search_range,h", po::value<int>()->default_value(1), "search range for DCCBS")
		("search_decaying_rate,r", po::value<int>()->default_value(0), "search decaying rate for DCCBS")
		("search_decaying_period,p", po::value<int>()->default_value(10), "search decaying period for DCCBS")
		("min_search_range,l", po::value<int>()->default_value(1), "minimum search range for DCCBS")
		;
	...
	else if (vm["cluster_heuristics"].as<string>() == "DCCBS")
		ch = cluster_heuristics_type::DCCBS;
    else
    {
	...
	// for DCCBS
	cbs.setSearchRange(vm["search_range"].as<int>());
	cbs.setSearchDecayingRate(vm["search_decaying_rate"].as<int>());
	cbs.setSearchDecayingPeriod(vm["search_decaying_period"].as<int>());
	cbs.setMinSearchRange(vm["min_search_range"].as<int>());
